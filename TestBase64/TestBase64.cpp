...
TEST(CheckDecoderThrowsOnInvalidAlphabetCharacters)
{
	...
	CHECK_THROW(std::for_each(encoded_string.cbegin(), encoded_string.cend(), decoder<std::string>(decoded_string)), std::runtime_error);
}
...
TEST(CheckDecoderHandlesZeroByteString)
{
	const std::string encoded_string("");
	std::string decoded_string;
	std::for_each(encoded_string.cbegin(), encoded_string.cend(), decoder<std::string>(decoded_string));
	CHECK_EQUAL(0U, decoded_string.size());
	CHECK_EQUAL("", decoded_string);
}
